...
@doc raw"""
    GeneralizedInverseGaussian(::Val{:conj}, a::Real, b::Real, p::Real=-1/2)

GIG with PDF:

```math
p(v) \propto v^{p-1} \exp\left( -\frac12 (a v^{-1} + bv) \right)
```
"""
GeneralizedInverseGaussian(::Val{:conj}, a::Real, b::Real, p::Real=-1/2) = GeneralizedInverseGaussian(sqrt(a/b), a, p)

Distributions.@distr_support GeneralizedInverseGaussian 0.0 Inf

Distributions.params(d::GeneralizedInverseGaussian) = (d.μ, d.λ, d.θ)
Distributions.partype(::GeneralizedInverseGaussian{T}) where T = T

Distributions.mode(d::GeneralizedInverseGaussian) = begin
    μ, λ, θ = params(d)
    tmp = μ * (θ - 1)
    μ / λ * (
        tmp + sqrt(λ^2 + tmp^2)
...
Distributions.mean(d::GeneralizedInverseGaussian) = begin
    μ, λ, θ = params(d)
    b0 = besselk(0 + θ, λ / μ)
    b1 = besselk(1 + θ, λ / μ)
    μ * b1 / b0
...
Distributions.var(d::GeneralizedInverseGaussian) = begin
    μ, λ, θ = params(d)
    b0 = besselk(0 + θ, λ / μ)
    b1 = besselk(1 + θ, λ / μ)
    b2 = besselk(2 + θ, λ / μ)
...
# Source: Wolfram
Distributions.skewness(d::GeneralizedInverseGaussian) = begin
    μ, λ, θ = params(d)
    b0 = besselk(0 + θ, λ / μ)
    b1 = besselk(1 + θ, λ / μ)
    b2 = besselk(2 + θ, λ / μ)
...
# Source: Wolfram
Distributions.kurtosis(d::GeneralizedInverseGaussian) = begin
    μ, λ, θ = params(d)
    t0 = besselk(0 + θ, λ/μ)
    t1 = besselk(1 + θ, λ/μ)
    t2 = besselk(2 + θ, λ/μ)
...
Distributions.logpdf(d::GeneralizedInverseGaussian, x::Real) = begin
    μ, λ, θ = params(d)
    if x >= 0
        -log(2) - θ * log(μ) - lbesselk(θ, λ / μ) - λ/2 * (1/x + x/μ^2) + (θ - 1) * log(x)
    else
        -Inf
    end
end

Distributions.cdf(d::GeneralizedInverseGaussian, x::Real) =
    if isinf(x)
        (x < 0) ? zero(x) : one(x)
    elseif isnan(x)
        typeof(x)(NaN)
...
Distributions.mgf(d::GeneralizedInverseGaussian, t::Number) = begin
    μ, λ, θ = params(d)
    tmp = 1 - 2t * μ^2 / λ
    tmp^(-θ/2) * besselk(θ, λ/μ * sqrt(tmp)) / besselk(θ, λ/μ)
end

Distributions.cf(d::GeneralizedInverseGaussian, t::Number) = mgf(d, 1im * t)
...
"""
Random.rand(rng::AbstractRNG, d::GeneralizedInverseGaussian) = begin
    # If X ~ GIG(μ, λ, θ), then X/μ ~ _GIG(λ/μ, θ),
    # so mu * _GIG(λ/μ, θ) == GIG(μ, λ, θ)
    μ, λ, θ = params(d)
    μ * Random.rand(rng, _GIG(λ/μ, θ))
end
...
Distributions.suffstats(::Type{<:GeneralizedInverseGaussian}, x::AbstractVector{<:Real}) =
    GeneralizedInverseGaussianStats(
        sum(log, x), sum(x), sum(inv, x), length(x)
    )

function Distributions.suffstats(::Type{<:GeneralizedInverseGaussian}, x::AbstractVector{T1}, w::AbstractVector{T2}) where {T1<:Real, T2<:Real}
    n = length(x)
    if length(w) != n
...
Distributions.params(d::_GIG) = (d.ω, d.λ)

Distributions.logpdf(d::_GIG, x::Real) =
    if x >= 0
        -log(2) - lbesselk(d.λ, d.ω) + (d.λ - 1) * log(x) - d.ω/2 * (x + 1/x)
    else
        -Inf
    end
...
1. Devroye, Luc. 2014. “Random Variate Generation for the Generalized Inverse Gaussian Distribution.” Statistics and Computing 24 (2): 239–46. https://doi.org/10.1007/s11222-012-9367-z.
"""
function Random.rand(rng::AbstractRNG, d::_GIG{<:Real, <:Real})
    # ω, λ = params(d)
    # negative_lmb = λ < 0
    # λ = abs(λ)
    ...
    # Generation
    U, V, W = Random.rand(rng), Random.rand(rng), Random.rand(rng)
    X = if U < q / (p + q + r)
        -s_ + q * V
    elseif U < (q + r) / (p + q + r)
        t_ - r * log(V)
